    ...
    <x-slot name="header">
        <div class="flex justify-between items-center sticky top-0 z-50">
            ...
            <button onclick="document.getElementById('btn-submit').click()"
                class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-bold text-sm shadow-md flex items-center gap-2 transition transform hover:scale-105">
                ...
                <div class="bg-white rounded-lg shadow-lg p-6 border-l-4 border-yellow-500">
                    ...
                    <div class="mb-6 border-b pb-4 bg-yellow-50 p-4 rounded-lg">
                        ...
                        <select name="global_header_type" x-model="global.header_type" @change="onHeaderChange()"
                            class="w-full md:w-1/2 border-gray-300 rounded font-bold text-gray-800 focus:ring-yellow-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        <div x-show="['standar', 'tatap_muka'].includes(global.header_type)">
                            ...
                            <textarea name="global_masa_sidang" x-model="global.masa_sidang" rows="2"
                                class="w-full text-sm border-gray-300 rounded focus:ring-yellow-500"></textarea>
                        </div>

                        <div x-show="['standar', 'tatap_muka'].includes(global.header_type)">
                            <label class="text-xs font-bold text-gray-500 uppercase">Dapil / Wilayah</label>
                            <textarea name="global_dapil" x-model="global.dapil" rows="2"
                                class="w-full text-sm border-gray-300 rounded focus:ring-yellow-500"></textarea>
                        </div>

                        <div x-show="['tatap_muka', 'kunjungan'].includes(global.header_type)"
                            class="col-span-1 md:col-span-2 bg-blue-50 p-3 rounded border border-blue-200">
                            <label class="text-xs font-bold text-blue-700 uppercase flex justify-between">
                                <span>Konten Deskripsi (Bebas Enter)</span>
                                <span class="text-gray-400 font-normal normal-case">Tekan Enter untuk baris baru</span>
                            </label>
                            <textarea name="global_deskripsi" x-model="global.deskripsi" rows="4"
                                class="w-full text-sm border-blue-300 rounded focus:ring-blue-500 font-bold text-gray-800"
                                placeholder="Ketik keterangan kegiatan disini..."></textarea>
                        </div>

                        <div class="col-span-1 md:col-span-2">
                            <label class="text-xs font-bold text-gray-500 uppercase">Tanggal (Footer)</label>
                            <input type="text" name="global_tanggal" x-model="global.tanggal"
                                class="w-full text-sm border-gray-300 rounded focus:ring-yellow-500 font-bold">
                        </div>
                    </div>
                        ...
                        <template x-for="(config, index) in masterConfig" :key="index">
                            <div class="flex items-center gap-2">
                                ...
                                <input type="text" x-model="config.title"
                                    class="flex-grow text-sm border-gray-300 rounded font-bold uppercase"
                                    placeholder="JUDUL HALAMAN">

                                <select x-model="config.layout" :disabled="global.header_type != 'standar'"
                                    class="text-sm border-gray-300 rounded w-40 disabled:bg-gray-200 disabled:text-gray-500">
                                    <option value="8">8 Kotak</option>
                                    <option value="6">6 Kotak</option>
                                    <option value="3">3 Kotak</option>
                                </select>

                                <button type="button" x-show="global.header_type == 'standar'"
                                    @click="removeMasterItem(index)"
                                    class="text-red-500 hover:text-red-700 p-1">Hapus</button>
                            </div>
                        </template>

                        <button type="button" x-show="global.header_type == 'standar'" @click="addMasterItem()"
                            class="text-sm text-blue-600 hover:underline font-bold mt-2">+ Tambah Halaman</button>
                    </div>
                        ...
                        <div class="bg-white shadow-2xl relative transition-all"
                            ...
                            <div class="text-center mb-4 font-tahoma text-12pt">
                                ...
                                <template x-if="global.header_type == 'tatap_muka'">
                                    <div>
                                        <div class="mb-1">Lampiran Foto</div>
                                        <div x-text="global.masa_sidang"></div>

                                        <div x-text="global.dapil"></div>

                                        <div class="font-bold uppercase" style="white-space: pre-wrap;"
                                            x-text="global.deskripsi"></div>
                            ...
                            <div class="grid grid-cols-2 gap-x-[5mm] gap-y-[5mm]"
                                ...
                                <template x-for="(photo, photoIndex) in sheet.photos" :key="photoIndex">
                                    <div class="relative border-2 border-black bg-gray-50 group/box hover:border-blue-500 transition overflow-hidden"
                                        ...
                                        <input type="hidden" :name="`sheets[${sheetIndex}][photos][]`"
                                            :value="photo">
                                        <template x-if="photo">
                                            <div class="w-full h-full relative">
                                                <img :src="photo" class="w-full h-full object-cover">
                                                <button type="button" @click="removePhoto(sheetIndex, photoIndex)"
                                                    class="absolute top-1 right-1 bg-white text-red-600 rounded-full p-1 opacity-0 group-hover/box:opacity-100 transition z-10 shadow"><svg
                                        ...
                                        <template x-if="!photo">
                                            <input type="file" accept="image/*"
                                                ...
                                                @change="handleSingleFile($event, sheetIndex, photoIndex)">
                                        </template>
                                    </div>
                                </template>
                            </div>
                            <div class="absolute bottom-2 right-6 text-gray-400 text-xs font-mono">Halaman <span
                                    x-text="sheetIndex + 1"></span></div>
                        </div>
                    </div>
                </template>
            </div>
